...
for iteration in range(simInitsTotal):  # can remove this loop, there is only 1 init 
# Note, I think it is confusing to call these 'initializations', since the 'data' is not reset to its starting confirmation
#   
    a = Simulation(N=N, 
                   error_tol=0.01, 
                   collision_rate=0.01, 
    ...
    offRate = 0.5
    contactRadius = 5
    colorStates =  np.tile(monomerTypes, [iters+1,1]) # initialize a matrix to store color states in. 
    for t in range(iters):
        ...
        for p in range(replicates):
            polyDat = data['pos'][p*monomers:(p+1)*monomers,:]
            newColors = colorStates[t+1,p*monomers:(p+1)*monomers] # note this is not a copy, just a reference. updating newColors immideately updates colorStates
            fracBound = sum(newColors)/monomers
            fracFree = max(0,totPc-fracBound)
            isLoss = np.random.rand(monomers) < offRate
            newColors[isLoss] = 0  # note, this immideately updates colorStates
            ...
            for o in ordr:
                ...
                if updateColor:
                    newColors[o] = 1  # note, this immideately updates colorStates
        ...
        a.add_force(
            polychrom.forcekits.polymer_chains(
                ...
                nonbonded_force_kwargs={
                    ...
                    'attractionRadius': attraction_radius,
                    'interactionMatrix': interactionMatrix,
                    'monomerTypes': colorStates[t+1,:],  # the updated colors 
                    'extraHardParticlesIdxs': []
                },
                bond_force_kwargs={
                    'bondLength': 1,
