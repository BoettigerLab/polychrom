...
TADs =  np.array(ast.literal_eval(parValues[4])) # [80,280,420,550,700,850] # positioned on HoxA
ctcfCapture = parValues[5]  # 0.9 80% capture probability 
ctcfRelease = parValues[6] # [0.003   =  hold it for ~300 blocks on average]
...
for i in range(M): # loop over chains (this variable needs a better name Max)
    for tad in TADs: # loop over boundaries (this name is Ok)
        pos = i * N1 + tad 
        ctcfLeftCapture[pos] = ctcfCapture  # 80% capture probability 
        ctcfLeftRelease[pos] = ctcfRelease  # hold it for ~300 blocks on average
        ctcfRightCapture[pos] = ctcfCapture
        ctcfRightRelease[pos] = ctcfRelease
...
with h5py.File(lefPosFile, mode='w') as myfile:
    ...
    for st,end in zip(bins[:-1], bins[1:]):
        ...
        for i in range(st, end):
            ex1D.translocate(cohesins, occupied, args)  # actual step of LEF dynamics 
